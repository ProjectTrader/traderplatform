var httpHelper = require('../helpers/https-helper.js'),
    ...
    strategies = require('../helpers/strategies.js'),
    google = require('../helpers/googleplus'),
		modelProvider = require('../db/models/modelProvider');
...
exports.setOAuthTokens = function(clientId, clientSecret, appMode, googleClientId, googleClientSecret,googleRedirectURL){
  CLIENT_ID = clientId;
...
exports.authenticate_google = function(){
    return function(req,res) {
      ...
      var onCreateUserSuccess = function(user){
          strategies.findOrCreateDefaultStrategiesForGoogle(user,onCreateGroup,onFail);
          req.session.user = user;
      };
                ...
                function(accessData){
                  ...
                  getGitHubProfile(accessData, function(user){
                      ...
                      var onFailGroups = function(err){
                        console.log(err,"Failed to create default strategy_list", user._id);
                      ...
                      var onEmailResoluionSuccess = function(user){
                        strategies.findOrCreateDefaultGroups(user, onDefaultGroupCreation, onFailGroups);
                      };
...
exports.delete_user = function(){
    return function(req, res){
      ...
      var deleteUser = function(err){
        User.remove(removeParm, function(err){
          if(err){
            handleErrors(err,res);
          }else{
            res.status(200).send();
      ...
      var deleteAllGroups = function(){
          Market.remove(removeParm, function(err){
            if(err){
              handleErrors(err,res);
            }else{
              deleteUser();
      ...
      var deleteAllSnippets = function(){
        Strategy.remove(removeParm, function(err){
          if(err){
            handleErrors(err,res);
          }else{
            deleteAllGroups();
